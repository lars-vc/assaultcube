...
enum // static entity types
{
    ...
    CLIP,     // attr1 = elevation, attr2 = xradius, attr3 = yradius, attr4 =
              // height, attr6 = slope, attr7 = shape
    PLCLIP,   // attr1 = elevation, attr2 = xradius, attr3 = yradius, attr4 =
              // height, attr6 = slope, attr7 = shape
    DUMMYENT, // temporary entity without any function - will not be saved to
              // map files, used to mark positions and for scripting
    MAXENTTYPES
};
#include "lars.h"
...
struct persistent_entity // map entity
{
    ...
    persistent_entity(short x, short y, short z, uchar type, short attr1,
                      uchar attr2, uchar attr3, uchar attr4, short attr5,
                      char attr6, unsigned char attr7)
        : x(x), y(y), z(z), attr1(attr1), type(type), attr2(attr2),
          attr3(attr3), attr4(attr4), attr5(attr5), attr6(attr6), attr7(attr7) {
    }
    persistent_entity(short x, short y, short z, uchar type, short attr1,
                      uchar attr2, uchar attr3, uchar attr4)
        : x(x), y(y), z(z), attr1(attr1), type(type), attr2(attr2),
          attr3(attr3), attr4(attr4), attr5(0), attr6(0), attr7(0) {}
    persistent_entity() {}
};
...
struct entity : persistent_entity {
    ...
    entity(short x, short y, short z, uchar type, short attr1, uchar attr2,
           uchar attr3, uchar attr4, short attr5, char attr6,
           unsigned char attr7)
        : persistent_entity(x, y, z, type, attr1, attr2, attr3, attr4, attr5,
                            attr6, attr7),
          spawned(false) {}
    entity(short x, short y, short z, uchar type, short attr1, uchar attr2,
...
class physent {
  public:
    ...
    bool inwater;
    bool onfloor, onladder, jumpnext, jumpd, crouching, crouchedinair,
        trycrouch, cancollide, stuck, scoping;
    int lastjump;
    ...
    physent()
        : o(0, 0, 0), deltapos(0, 0, 0), newpos(0, 0, 0), yaw(270), pitch(0),
          roll(0), pitchvel(0), crouching(false), crouchedinair(false),
          trycrouch(false), cancollide(true), stuck(false), scoping(false),
          lastjump(0), lastjumpheight(200), lastsplash(0), state(CS_ALIVE),
          last_pos(0) {
        reset();
    }
    virtual ~physent() {}
    ...
    void reset() {
        ...
        onfloor = onladder = inwater = jumpnext = jumpd = crouching =
            crouchedinair = trycrouch = stuck = false;
        last_pos = 0;
    }
...
class playerstate {
  public:
    ...
    bool canpickup(int type) {
        switch (type) {
        case I_CLIPS:
            return ammo[akimbo && ammo[GUN_AKIMBO] < ammostats[GUN_AKIMBO].max
                            ? GUN_AKIMBO
                            : GUN_PISTOL] <
                   ammostats[akimbo && ammo[GUN_AKIMBO] <
                                           ammostats[GUN_AKIMBO].max
                                 ? GUN_AKIMBO
                                 : GUN_PISTOL]
                       .max;
        case I_AMMO:
        ...
        case I_GRENADE:
            return get_val_lars(mag[GUN_GRENADE]) < ammostats[GUN_GRENADE].max;
        case I_HEALTH:
    ...
    void pickup(int type) {
        ...
        case I_GRENADE:
            ...
            // lars annoying stuff
            // additem(ammostats[GUN_GRENADE], get_val(mag[GUN_GRENADE]));
            break;
        case I_HEALTH:
        ...
        case I_AKIMBO:
            akimbo = true;
            set_val_lars(mag[GUN_AKIMBO], guns[GUN_AKIMBO].magsize);
            additem(ammostats[GUN_AKIMBO], ammo[GUN_AKIMBO]);
            break;
        }
    }
    ...
    void respawn() {
        ...
        akimbo = false;
        loopi(NUMGUNS) ammo[i] = gunwait[i] = 0;
        loopi(NUMGUNS) set_val_lars(mag[i], 0);
        ammo[GUN_KNIFE] = 1;
        set_val_lars(mag[GUN_KNIFE], 1);
    }
    ...
    virtual void spawnstate(int gamemode) {
        ...
        if (!m_nopistol) {
            ...
            set_val_lars(mag[GUN_PISTOL], magsize(GUN_PISTOL));
        }
...
class playerent : public dynent, public playerstate {
  ...
  public:
    ...
    int ffov, scopefov;
    bool allowmove() {
        return (state != CS_DEAD && state != CS_SPECTATE) ||
               spectatemode == SM_FLY;
    }
    ...
    playerent()
        : curskin(0), clientnum(-1), lastupdate(0), plag(0), ping(0),
          address(0), lifesequence(0), frags(0), flagscore(0), deaths(0),
          tks(0), lastpain(0), lastvoicecom(0), lastdeath(0),
          clientrole(CR_DEFAULT), team(TEAM_SPECT), spectatemode(SM_NONE),
          followplayercn(FPCN_VOID), eardamagemillis(0), maxroll(ROLLMOVDEF),
          maxrolleffect(ROLLEFFDEF), movroll(0), effroll(0), ffov(0),
          scopefov(0), prevweaponsel(NULL), weaponsel(NULL),
          nextweaponsel(NULL), primweap(NULL), nextprimweap(NULL),
          lastattackweapon(NULL), smoothmillis(-1), head(-1, -1, -1),
          ignored(false), muted(false), nocorpse(false) {
        type = ENT_PLAYER;
        name[0] = 0;
        maxeyeheight = 4.5f;
        aboveeye = 0.7f;
    ...
    void selectweapon(int w, bool other = false) {
        ...
        if (!prevweaponsel)
            prevweaponsel = weaponsel;
        if (other && (prevweaponsel != weaponsel))
            weaponswitch(weaponsel, true);
    }
    ...
    void startmap() { frags = flagscore = deaths = lifesequence = tks = 0; }
};
