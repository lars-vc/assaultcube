...
vec CBot::GetEnemyPos(playerent *d) {
    ...
    vec o = m_pMyEnt->weaponsel->type == GUN_SNIPER && d->head.x >= 0 ? d->head
                                                                      : d->o,
        offset;
...
// WIP
bool CBot::BotsAgainstHumans() { return false; }
...
bool CBot::FindEnemy(void) {
    ...
    if (m_pMyEnt->enemy) // Bot already has an enemy
    {
        ...
        // Check if the enemy is still ingame, still alive, not joined my team
        // and is visible
        if (found && !isteam(m_pMyEnt->team, m_pMyEnt->enemy->team)) {
            if ((m_pMyEnt->enemy->state == CS_ALIVE) &&
    ...
    if (m_iEnemySearchDelay > lastmillis)
        return (m_pMyEnt->enemy != NULL);
    ...
    m_iEnemySearchDelay =
        lastmillis + int(RandomFloat(MinDelay, MaxDelay) * 1000.0f);
...
bool CBot::CheckHunt(void) {
    ...
    if (m_pHuntTarget) // Bot already has an enemy to hunt
    {
        ...
        if (found && !isteam(m_pMyEnt->team, m_pHuntTarget->team)) {
            if ((m_pHuntTarget->state == CS_ALIVE) &&
                IsReachable(m_vHuntLocation))
                return true;
        } else
            m_pHuntTarget = NULL;
    }

    if (m_iHuntDelay > lastmillis)
        return (m_pHuntTarget != NULL);
    ...
    playerent *pNewEnemy = NULL, *d = NULL;
    float flDist, flNearestDist = 99999.9f,
                  flNearestOldPosDistToEnemy = 99999.9f;
        ...
        if (player1 && !isteam(player1->team, m_pMyEnt->team) &&
            ...
            if (EnemyVal >= BestEnemyVal) {
                ...
                for (int j = 0; j < d->history.size(); j++) {
                    ...
                    if ((flDist < flNearestOldPosDistToEnemy) &&
                        IsReachable(v)) {
                        flNearestOldPosDistToEnemy = flDist;
                        bestfromenemy = v;
                    }
                ...
                for (int j = 0; j < m_pMyEnt->history.size(); j++) {
                    ...
                    if ((flDist < flNearestOldPosDistToBot) &&
                        ::IsVisible(d->o, v) && IsReachable(v)) {
                        flNearestOldPosDistToBot = flDist;
                        bestfrombot = v;
                    }
                }
...
bool CBot::HuntEnemy(void) {
    ...
    if ((m_iHuntPlayerUpdateTime < lastmillis) || bDone) {
        ...
        for (j = 0; j < m_pMyEnt->history.size(); j++) {
            ...
            if ((flDist < NearestDistToBot) &&
                ::IsVisible(m_pHuntTarget->o, OldPos) &&
                (IsReachable(OldPos))) {
        ...
        if (BestPosIndexFromEnemy > -1) {
            m_vPrevHuntLocation = m_vHuntLocation;
            m_vHuntLocation =
                m_pHuntTarget->history.getpos(BestPosIndexFromEnemy);
            bNew = true;
            m_fPrevHuntDist = 0.0f;
        ...
        if (m_iLookAroundDelay < lastmillis) {
            if (m_iLookAroundTime > lastmillis) {
                if (m_iLookAroundUpdateTime < lastmillis) {
                    ...
                    m_pMyEnt->targetyaw =
                        WrapYZAngle(m_pMyEnt->targetyaw + flAddAngle);
    ...
    if ((m_iHuntLastTurnLessTime > 0) &&
        (m_iHuntLastTurnLessTime < (lastmillis - 1000))) {
...
void CBot::CheckWeaponSwitch() {
    if (m_pMyEnt->nextweaponsel == NULL)
        m_pMyEnt->weaponchanging = 0;
    if (!m_pMyEnt->weaponchanging)
        return;
...
void CBot::ShootEnemy() {
    ...
    {
        if (m_pMyEnt->mag[m_pMyEnt->gunselect].val()) {
            // If the bot is using a sniper only shoot if crosshair is near the
            // enemy
            if (WeaponInfoTable[m_pMyEnt->gunselect].eWeaponType ==
                TYPE_SNIPER) {
                float yawtoturn =
                    fabs(WrapYZAngle(m_pMyEnt->yaw - m_pMyEnt->targetyaw));
                float pitchtoturn =
                    fabs(WrapYZAngle(m_pMyEnt->pitch - m_pMyEnt->targetpitch));

                if ((yawtoturn > 5) ||
                    (pitchtoturn > 15)) // UNDONE: Should be skill based
                    return;
            }
            ...
            if ((flDist <
                 WeaponInfoTable[m_pMyEnt->gunselect].flMinFireDistance) ||
                (flDist >
                 WeaponInfoTable[m_pMyEnt->gunselect].flMaxFireDistance))
                return;
...
bool CBot::ChoosePreferredWeapon() { return true; }
...
{
    if (m_pMyEnt->mag[m_pMyEnt->weaponsel->type].val() >=
        WeaponInfoTable[m_pMyEnt->weaponsel->type].sMinDesiredAmmo)
        return; // do not reload if mindesiredammo is satisfied
    if (m_pMyEnt->enemy && m_pMyEnt->mag[m_pMyEnt->weaponsel->type].val()) {
...
#define MINSCOPETIME 1000
    if (m_pMyEnt->weaponsel->type != GUN_SNIPER)
        return;
    sniperrifle *sniper = (sniperrifle *)m_pMyEnt->weaponsel;
    if (m_pMyEnt->enemy &&
        m_pMyEnt->o.dist(m_pMyEnt->enemy->o) > MINSCOPEDIST) {
...
void CBot::MainAI() {
    ...
    // Heading to an interesting entity(ammo, armour etc)
        ...
        m_eCurrentBotState = STATE_ENT;
               ...
               DoSPStuff()) // Home to goal, find/follow friends etc.
    {
    ...
    } else // Normal navigation
    {
        ...
        // if it is time to look for a waypoint AND if there are waypoints in
        // this level...
        ...
        if (bDoNormalNav) {
            ...
            // Time to check the environment?
                ...
                if (m_vWaterGoal != g_vecZero)
                    m_vWaterGoal = g_vecZero;

                // Check for stuck and strafe
                if (UnderWater(m_pMyEnt->o) || !CheckStuck()) {
                    // Only do this when the bot is underwater or when the bot
                    // isn't stuck
...
void CBot::DoCombatNav() {
    ...
    if ((fabs(m_pMyEnt->enemy->o.x - m_pMyEnt->o.x) < r &&
        ...
        ((m_pMyEnt->enemy->o.z + m_pMyEnt->enemy->aboveeye) <
         (m_pMyEnt->o.z + m_pMyEnt->aboveeye))) {
        // Try to get off him!
        condebug("On enemy!");
    ...
    if (((m_iCheckEntsDelay < lastmillis) || m_pTargetEnt) &&
        ...
        if (m_pTargetEnt) {
            ...
            vec v(m_pTargetEnt->x, m_pTargetEnt->y,
                  S(m_pTargetEnt->x, m_pTargetEnt->y)->floor +
                      m_pMyEnt->eyeheight);
        ...
        if (!m_pTargetEnt && (m_iCheckEntsDelay <= lastmillis)) {
            if (WeaponInfoTable[m_pMyEnt->gunselect].eWeaponType == TYPE_MELEE)
                ...
                bSearchItems =
                    (m_pMyEnt->ammo[m_pMyEnt->gunselect] <=
...
#endif
                        m_pMyEnt->jumpnext =
                            true; // Jump if close to ent and the ent is high
                    } else
                        bToHigh = true;
                }
            }
    ...
    if ((m_sSkillNr <= 1) && (m_fYawToTurn < 80.0f) && (flDist <= 20.0f) &&
        ...
        if (RandomLong(1, 100) <= sJumpPercent) {
            ...
            if ((GetDistance(m_pMyEnt->enemy->o) <= 4.0f) ||
                (WeaponInfoTable[m_pMyEnt->gunselect].eWeaponType ==
                 TYPE_MELEE)) {
    ...
    if ((flDist <= WeaponInfoTable[m_pMyEnt->gunselect].flMinDesiredDistance) ||
        ...
        vec src, forward, right, up, dest, MyAngles = GetViewAngles(),
                                           o = m_pMyEnt->o;
        traceresult_s tr;
        ...
        if (m_iMoveDir != DIR_NONE) {
            ...
            m_iCombatNavTime = lastmillis + 500;
                   ...
                   ->bCircleStrafe) // Circle strafe when in desired range...
    {
        traceresult_s tr;
        ...
        angles.y =
            WrapYZAngle(angles.y - 75.0f); // Not 90 degrees because the bot
...
bool CBot::CheckStuck() {
    ...
    if (IsStuck) {
        ...
        if (RandomLong(0, 2) == 0)
            ...
            m_pMyEnt->targetyaw =
                WrapYZAngle(m_pMyEnt->yaw + RandomLong(60, 160));
        return true;
    }
...
bool CBot::CheckJump() {
    ...
    if (!IsVisible(start, FORWARD, flTraceDist, false)) {
        ...
        if (IsVisible(start) &&
            !IsVisible(start, FORWARD, flTraceDist, false)) {
            // Jump
            debugnav("High wall");
            m_pMyEnt->jumpnext = true;
            return true;
        ...
        // Check if something is blocking at feet height, so the bot can jump
        // over it
        start.z -= 1.7f;
...
bool CBot::CheckCrouch() {
    ...
    if (!IsVisible(vec(start).add(crouch), FORWARD, flTraceDist, false) &&
        IsVisible(vec(start).sub(crouch), FORWARD, flTraceDist, false))
        return true;
    return false;
}
...
void CBot::CheckFOV() {
    ...
    if (!UnderWater(m_pMyEnt->o) && m_pMyEnt->onfloor) {
        ...
        if (flNearestHitDist < 4) {
            ...
            // Check if something is blocking below us, so the bot can jump over
            // it
            src = MyAngles;
            src.x = 0;
        ...
        for (int x = cx - 1; x <= (cx + 1); x++) {
            for (int y = cy - 1; y <= (cy + 1); y++) {
                ...
                // if (!SOLID(s))
                {
                    ...
                    bool turn = false;
                    if (UnderWater(tr.end))
                        CubesInWater++;
                    if (CubesInWater > 2)
                        turn = true; // Always avoid water
                    if (!tr.collided && RandomLong(0, 1))
                        turn = true; // Randomly avoid a fall

                    if (turn) {
                        m_pMyEnt->targetyaw = WrapYZAngle(m_pMyEnt->yaw + 180);
                        m_iCheckEnvDelay = m_iStrafeCheckDelay =
                            lastmillis + RandomLong(750, 1500);
    ...
    if (flNearestHitDist <= 4.0f) {
        ...
        if (WallLeft && tr.collided) {
            ...
            debugnav("Corner");
            m_pMyEnt->targetyaw =
                WrapYZAngle(m_pMyEnt->yaw + RandomFloat(160.0f, 200.0f));
            m_iCheckEnvDelay = m_iStrafeCheckDelay =
                lastmillis + RandomLong(750, 1500);
            return;
        }
    ...
    if (m_vWaterGoal == g_vecZero) {
        ...
        for (int x = (cx - iSearchRange); x <= (cx + iSearchRange); x++) {
            for (int y = (cy - iSearchRange); y <= (cy + iSearchRange); y++) {
                ...
                if (UnderWater(v))
                    continue; // Skip, cube is underwater

                if (waterlevel < (v.z - 2.0f))
                    continue; // Cube is too high
        ...
        if (IsVisible(to)) {
            ...
            /*
                        if (to.z > (from.z + JUMP_HEIGHT))
                        {
                            ...
                            // check if we didn't hit anything, if so then it's
               in mid-air if (::IsVisible(v_new_src, v_new_dest, NULL))
                            {
...
void CBot::HearSound(int n, vec *o) {
    ...
    if (n == S_DIE1 || n == S_DIE2)
        return;

    int soundvol =
        m_pBotSkill->iMaxHearVolume -
    ...
    if (pNearest) {
        if (m_pMyEnt->enemy != pNearest)
            m_iShootDelay =
                lastmillis +
                GetShootDelay(); // Add shoot delay when new enemy found
