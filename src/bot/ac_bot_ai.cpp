...
weaponinfo_s WeaponInfoTable[MAX_WEAPONS] = {
    ...
    {TYPE_SNIPER, 15.0f, 80.0f, 3.0f, 180.0f,
     2}, // CARBINE    // min and max FD just guesstimates for now
...
bool CACBot::ChoosePreferredWeapon() {
    ...
    for (int i = 0; i < MAX_WEAPONS; i++) {
        ...
        if (!get_val_lars(m_pMyEnt->mag[i]) &&
            WeaponInfoTable[i].eWeaponType != TYPE_MELEE) {
            continue;
        } else {
            sWeaponScore += 5;
        }
        ...
        if ((flDist >= WeaponInfoTable[i].flMinDesiredDistance) &&
            ...
            if (i == GUN_PISTOL ||
                WeaponInfoTable[i].eWeaponType == TYPE_MELEE) {
                if (WeaponInfoTable[m_pMyEnt->primary].eWeaponType ==
                    TYPE_SNIPER) {
                    sWeaponScore +=
                        10; // At a close range, knife & pistol are strong with
                            // a sniper like primary weapon
                }
                if (WeaponInfoTable[i].eWeaponType == TYPE_MELEE &&
                    WeaponInfoTable[m_pMyEnt->primary].eWeaponType ==
                        TYPE_SHOTGUN) {
                    sWeaponScore -=
                        2; // Penalize a bit knife on close range with shotgun
                }
            }
        ...
        // Score on the difference of the avarage of the Min and Max desired
        // distance.
        ...
        if (WeaponInfoTable[i].sMinDesiredAmmo > 0) {
            ...
            float flDesiredPercent =
                (float(m_pMyEnt->ammo[i]) /
    ...
    */
    ...
    loopv(ents) {
        ...
        case I_GRENADE:
            ...
            bInteresting =
                (get_val_lars(m_pMyEnt->mag[GUN_GRENADE]) < sMaxAmmo);
            sAmmo = -1;
            break;
        case I_HEALTH:
    ...
    if (pNewTargetEnt) {
        ...
        m_vGoal.z =
            S(pNewTargetEnt->x, pNewTargetEnt->y)->floor + player1->eyeheight;
    }
...
bool CACBot::HeadToTargetEnt() {
    if (m_pTargetEnt) {
        ...
        if (m_pTargetEnt->spawned &&
            (!UnderWater(m_pMyEnt->o) || !UnderWater(o))) {
...
bool CACBot::DoSPStuff() { return false; }
