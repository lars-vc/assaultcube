...
/*
     ...
     -If a bot gets ammo it should ignore that kind of ammo for the next 10
   seconds. ? -bots should ignore players that are too far away ? -bots should
   dodge/jump when shot at -If the bot's only weapon is the fist it should make
   looking for ammo its priority - Done ? -Make all the "UNDONE's" done :)
...
#else
inline void condebug(const char *s, int a = 0, int b = 0,
                     int c = 0) { /*conoutf(s, a, b, c);*/
}
inline void debugnav(const char *s, int a = 0, int b = 0,
                     int c = 0) { /*conoutf(s, a, b, c);*/
}
...
struct bot_skill_s {
    ...
    float flMinEnemySearchDelay; // Minimal delay for when a bot can search for
                                 // an enemy again
    float flMaxEnemySearchDelay; // Maximal delay for when a bot can search for
                                 // an enemy again
...
class CBot {
  public:
    ...
    int m_iWaypointHeadLastTurnLessTime; // Last time the didn't had to turn
                                         // much wile heading to a WP
    ...
    bool IsVisible(playerent *d, bool CheckPlayers = false) {
        return ::IsVisible(m_pMyEnt->o, d->o, (CheckPlayers) ? m_pMyEnt : NULL);
    };
    bool IsVisible(entity *e, bool CheckPlayers = false);
    bool IsVisible(vec o, int Dir, float flDist, bool CheckPlayers,
                   float *pEndDist = NULL);
...
class CStoredBot // Used to store bots after mapchange, so that they can be
                 // readded
{
  public:
    char m_szName[32];
...
class CBotManager {
    char m_szBotNames[150]
                     [16]; // Max 150 bot names with a length of 16 characters
    ...
    TLinkedList<CStoredBot *>
        m_StoredBots; // List of bots that should be re-added after map change
  ...
  public:
    ...
    void MakeBotFileName(const char *szFileName, const char *szDir1,
                         const char *szDir2, char *szOutput);
};
