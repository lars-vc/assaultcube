...
void selectweapon(weapon *w) {
    if (!w || !player1->weaponsel->deselectable() || ispaused)
        return;
    if (w->selectable()) {
...
void tryreload(playerent *p) {
    if (!p || p->state != CS_ALIVE || ispaused || p->weaponsel->reloading ||
        p->weaponchanging)
        return;
...
inline bool
intersecttriangle(const vec &from, const vec &dir, const vec &v0, const vec &v1,
                  const vec &v2, vec *end,
                  float *_t) // precise but rather expensive, based on
            ...
            if (t > 0)
                loopk(2) {
                    ...
                    if (SOLID(s) || s->floor > p.z || s->ceil < p.z ||
                        s->type == CORNER) { // cube s is a probable hit,
                                             // examine further
                        ...
                        if (s->type == CORNER) {
                            ...
                            vec newto;
                            float newdist;
                            if (intersectcorner(from, d, bx, by, bs, !(q & 1),
                                                &newto, &newdist) &&
                                newdist < distmin &&
                                (!h || newto.z < ns->floor ||
                                 newto.z > ns->ceil))
                                to = newto, distmin = newdist;
                            if (d.z) { // intersect with z == const plane where
                                       // the corner ends
                                ...
                                vec pz = d;
                                pz.mul(tz).add(from);
                                if (pz.x >= bx && pz.x <= bx + bs &&
                                    pz.y >= by && pz.y <= by + bs &&
                                    tz < distmin)
                                    to = pz, distmin = tz;
                                if (h) { // intersect with the triangles where
                                         // socket corners end
                                    float sockplate =
                                        downwards ? ns->floor : ns->ceil;
                            ...
                            if (SOLID(s) || s->type == SPACE ||
                                ...
                                loopi(4) vdelta[i] =
                                    ((nb[i] = s + hfnb[i]))->vdelta; // 23
                                int nb2 = "\002\001\013\023"[xy + 2 * k],
                                    nb1 = nb2 >> 3;
                                nb2 &= 3;
                                if (s->type == FHF) { // FHF side surfaces
                                    float a = s->floor - vdelta[nb1] / 4.0f,
                                          b = (vdelta[nb1] - vdelta[nb2]) /
                                              4.0f,
                                          dummy,
                            ...
                            if (s->type == CHF || s->type == FHF)
                                loopi(3) if (vdelta[3] != vdelta[i]) isflat =
                                    false;
                            float h = downwards
                                          ? s->floor - (isflat && s->type == FHF
                                                            ? vdelta[0] / 4.0f
                                                            : 0)
                                          : s->ceil + (isflat && s->type == CHF
                                                           ? vdelta[0] / 4.0f
                                                           : 0);
                            if (isflat || (downwards && s->type == CHF) ||
                                (!downwards &&
                                 s->type == FHF)) { // intersect with z == const
                                                    // plane on either SPACE or
                                                    // flat ends of FHF and CHF
                                float tz =
                                    d.z != 0.0f ? (h - from.z) / d.z : distmin;
                                vec pz = d;
                                pz.mul(tz).add(from);
                                if (int(pz.x) == x && int(pz.y) == y &&
                                    tz < distmin)
                                    to = pz, distmin = tz;
                            }
                            if (!isflat) { // sloped CHF or FHF: check two
                                           // triangles, regardless of
                                           // "downwards" or not (FHF slopes can
                                           // be seen looking upwards)
                                if (s->type == FHF)
                                ...
                                vec newto;
                                if (s->type == FHF) {
                                    if (intersecttriangle(from, d, v0, v1, v2,
                                                          &newto, &newdist) &&
                                        newdist < distmin)
                                        to = newto, distmin = newdist;
                                    if (intersecttriangle(from, d, v3, v1, v2,
                                                          &newto, &newdist) &&
                                        newdist < distmin)
                                        to = newto, distmin = newdist;
                                } else {
                                    if (intersecttriangle(from, d, v0, v1, v3,
                                                          &newto, &newdist) &&
                                        newdist < distmin)
                                        to = newto, distmin = newdist;
                                    if (intersecttriangle(from, d, v2, v0, v3,
                                                          &newto, &newdist) &&
                                        newdist < distmin)
                                        to = newto, distmin = newdist;
                                }
...
void radialeffect(playerent *o, vec &v, int qdam, playerent *at, int gun) {
    ...
    if (dist < EXPDAMRAD) {
        if (at == player1 && o != player1 && multiplayer(NULL))
...
void shorten(const vec &from, vec &target, float distsquared) {
    ...
    if (m < 0.07f)
        target.add(
            vec(0.1f, 0.1f, 0.1f)); // if "from == target" just fake a target
                                    // to avoid zero-length fx vectors
    else
...
void raydamage(vec &from, vec &to, playerent *d) {
    ...
    if (d->weaponsel->type == GUN_SHOTGUN) {
        ...
        loopk(3) loopi(SGRAYS) {
            rayscount++;
            int h = k * SGRAYS + i;
            if ((hits[h] = intersectclosest(from, sgr[h].rv, d, distsquared,
                                            hitzone)))
        ...
        loopk(3) loopi(SGRAYS) {
            ...
            if (hits[h]) {
                ...
                dmg4r +=
                    (SGCOdmg / 10.0f * SGDMGTOTAL / 100.0f) * numhits_o / 21.0f;
                dmg4r +=
                    (SGCMdmg / 10.0f * SGDMGTOTAL / 100.0f) * numhits_m / 21.0f;
                dmg4r +=
                    (SGCCdmg / 10.0f * SGDMGTOTAL / 100.0f) * numhits_c / 21.0f;
...
void r_accuracy(int h) {
    ...
    if (accuracy && valid_weapon(i)) {
        ...
        blendbox(x_offset, y_offset + FONTH,
                 x_offset + text_width(line) + 2 * FONTH, y_offset - FONTH,
                 false, -1);
...
void accuracyinfo() {
    ...
    loopi(NUMGUNS) if (accuracym[i].shots) {
        ...
        string line;
        if (i == GUN_GRENADE || i == GUN_SHOTGUN) {
            formatstring(line)("\f0%-10s\t\f5 %.1f%% (%.1f/%d)", weapstr(i),
                               acc, accuracym[i].hits, accuracym[i].shots);
        } else {
...
void grenadeent::activate(const vec &from, const vec &vel) {
    ...
    if (local) {
        addmsg(SV_SHOOT, "ri2i3i", millis, owner->weaponsel->type,
               ...
               0); // server reads all weapons with DMF precision
...
void grenadeent::_throw(const vec &from, const vec &vel) {
    ...
    if (local) {
        addmsg(SV_THROWNADE, "ri7", int(o.x * DNF), int(o.y * DNF),
               int(o.z * DNF), int(vel.x * DNF), int(vel.y * DNF),
               int(vel.z * DNF), lastmillis - millis);
...
grenades::grenades(playerent *owner)
    : weapon(owner, GUN_GRENADE), inhandnade(NULL), throwwait((13 * 1000) / 40),
      throwmillis(0), cookingmillis(0), state(GST_NONE) {}
...
void grenades::attackfx(const vec &from, const vec &vel,
...
void grenades::thrownade() {
    ...
    vec lvel(sinf(RAD * owner->yaw) * speed, -cosf(RAD * owner->yaw) * speed,
             sinf(RAD * owner->pitch));
    lvel.mul(1.5f);
    vec uvel(int(lvel.x * DNF) / DNF, int(lvel.y * DNF) / DNF,
             int(lvel.z * DNF) / DNF); // force universal granularity
    thrownade(uvel);
}

void grenades::thrownade(const vec &vel) {
    inhandnade->moveoutsidebbox(vel, owner);
    // sending with DNF precision fixes current "remote stuck on plateau"-bug,
    // but "universal granularity"(TM) lessens discrepancies between worlds even
    // more
...
void sniperrifle::onselecting(bool sound) {
    weapon::onselecting(sound);
    scoped = false;
    player1->scoping = false;
}
...
void checkakimbo() {
    if (player1->akimbo) {
        ...
        bool isdeadorspect =
            player1->state == CS_DEAD || player1->state == CS_SPECTATE;
