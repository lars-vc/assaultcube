...
void loadcrosshair(const char *type, const char *filename) {
    ...
    if (!*filename) { // special form: loadcrosshair filename   // short for
        ...
        if (strcasecmp(type, "reset"))
            ...
            crosshairs[CROSSHAIR_TEAMMATE] =
                loadcrosshairtexture("teammate.png");
...
void drawdmgindicator(playerent *p) {
    ...
    loopi(8) {
        if (!damagedirections[i] || damagedirections[i] < lastmillis)
            continue;
        float t =
            damageindicatorsize / (float)(damagedirections[i] - lastmillis);
        glPushMatrix();
...
void drawequipicons(playerent *p) {
    ...
    if (p->mag[GUN_GRENADE].val())
        drawequipicon(
            oldfashionedgunstats
                ? (HUDPOS_GRENADE +
                   (((float)screenw / (float)screenh > 1.5f) ? 75 : 25)) *
                      2
                : HUDPOS_GRENADE * 2,
            1650, 3, 1);
...
void drawradar_showmap(playerent *p, int w, int h) {
    ...
    if (m_flags_) {
        ...
        loopi(2) // flag items
        {
            ...
            entity *e = f.flagent;
            bool hasflagent =
                e && e->x != -1 && e->y != -1; // no base for flagentitydummies
                                               // (HTF on maps without bases)
            if (hasflagent) {
        ...
        loopi(2) // flag items
        {
            ...
            if ((f.state == CTFF_INBASE && hasflagent) ||
                ...
                if (cpos.sqrxy() < d2s) {
                    ...
                    drawradarent(
                        cpos.x + offx, cpos.y - offy, camera1->yaw, 3,
                        m_ktf ? 2 : f.team, iconsize,
...
VARP(overviewflags, 0, 1,
     2); // SM_OVERVIEW flag rendering: 0:regular, 1:model-askew 2:radarents
string gtime;
int dimeditinfopanel = 255;

const char *ghoststrings[] = {
    "none",          "deathcam", "chase 1st", "chase 3rd [O]",
    "chase 3rd [A]", "fly",      "overview",  ""};
...
void gl_drawhud(int w, int h, int curfps, int nquads, int curvert,
    ...
    // glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA); // without below
    // condition it was always set in drawdmgindicator()
    if (damageindicatorplayer == p ||
        (is_spect &&
         getclient(player1->followplayercn) == damageindicatorplayer))
        drawdmgindicator(p);

    if (p->state == CS_ALIVE && !hidehudequipment)
        drawequipicons(p);

    if ((!hideradar || showmap) && player1->spectatemode <= SM_FLY)
        drawradar(p, w, h);
    if (!editmode) {
        glMatrixMode(GL_MODELVIEW);
    ...
    if (wallclock)
        filtertext(ltime,
                   timestring(*ltimeformat != 'U',
                              ltimeformat + int(*ltimeformat == 'U')),
                   FTXT_TOLOWER);
    ...
    if (showstats) {
        if (showstats == 2 && !dbgpos) {
            ...
            formatstring(text)(
                "\f%c%d", rangecolor(curfps, "xwvu", 30, 100, 150), curfps);
            draw_text(text, ttll - text_width(text), top);
            formatstring(text)("\f%c%d", rangecolor(lf, "uvwx", 199, 299, 399),
                               lf);
            draw_text(text, ttll - text_width(text), top + 80);
            formatstring(text)(
                "\f%c%d", rangecolor(nquads, "uvwx", 3999, 5999, 7999), nquads);
            draw_text(text, ttll - text_width(text), top + 160);
            formatstring(text)("\f%c%d",
                               rangecolor(curvert, "uvwx", 3999, 5999, 7999),
                               curvert);
            draw_text(text, ttll - text_width(text), top + 240);
            formatstring(text)("\f%c%d",
                               rangecolor(xtraverts, "uvwx", 3999, 5999, 7999),
                               xtraverts);
            draw_text(text, ttll - text_width(text), top + 320);

            if (wallclock)
                draw_text(ltime, ttll - text_width(ltime), top - 90);
            if (unsavededits)
            ...
            if (dbgpos) {
                ...
                draw_text(text, VIRTW * 2 - (text_width(text) + FONTH),
                          VIRTH * 2 - 17 * FONTH / 2);
                formatstring(text)("%05.2f PIT", camera1->pitch);
                draw_text(text, VIRTW * 2 - (text_width(text) + FONTH),
                          VIRTH * 2 - 15 * FONTH / 2);
                formatstring(text)("%05.2f X  ", camera1->o.x);
                draw_text(text, VIRTW * 2 - (text_width(text) + FONTH),
                          VIRTH * 2 - 13 * FONTH / 2);
                formatstring(text)("%05.2f Y  ", camera1->o.y);
                draw_text(text, VIRTW * 2 - (text_width(text) + FONTH),
                          VIRTH * 2 - 11 * FONTH / 2);
                formatstring(text)("%05.2f Z  ", camera1->o.z);
                draw_text(text, VIRTW * 2 - (text_width(text) + FONTH),
                          VIRTH * 2 - 9 * FONTH / 2);
                popfont();
            }
    ...
    if (editmode && !hideeditinfopanel) {
        ...
        if (editip && dimeditinfopanel) {
            ...
            draw_text(editip, VIRTW * 2 - w - FONTH, VIRTH * 13 / 8 - h, 255,
                      255, 255, dimeditinfopanel);
        }
    ...
    if (!intermission && lastgametimeupdate != 0) {
        int cssec =
            (gametimecurrent + (lastmillis - lastgametimeupdate)) / 1000;
        int gtsec = cssec % 60;
        int gtmin = cssec / 60;
    ...
    if (menu)
        ...
        int offsetx = 40 + ((editmode && (showeditingsettings >= 2 ||
                                          keepshowingeditingsettingstill))
                                ? ((showeditingsettings == 2 &&
    ...
    if (ispaused) {
        ...
        glOrtho(0, VIRTW * matchpausedfontfactor, VIRTH * matchpausedfontfactor,
                0, -1, 1);
        const int left =
            (VIRTW * matchpausedfontfactor) / 2 - text_width(matchpaused) / 2;
    ...
    if (!hidespecthud && spectating && player1->spectatemode != SM_DEATHCAM) {
        ...
        int smprev = player1->spectatemode > SM_FOLLOW1ST
                         ? (player1->spectatemode - 1)
                         : lastvalid;
        int smnext = player1->spectatemode < lastvalid
                         ? (player1->spectatemode + 1)
                         : SM_FOLLOW1ST;
        int gtoffx = 250; // static offset /hoping/ to be longer than the
                          // longest(!font?) specttext
        draw_textf("\fZ%s", virtposx + gtoffx, virtposy * 7 - FONTH,
                   ghoststrings[smprev]);
        draw_textf("\fY%s", virtposx + gtoffx, virtposy * 7,
                   ghoststrings[player1->spectatemode]);
        draw_textf("\fZ%s", virtposx + gtoffx, virtposy * 7 + FONTH,
                   ghoststrings[smnext]);
    ...
    if (p->state == CS_ALIVE ||
        ...
        if ((m_flags_ || m_teammode) && !hideteamscorehud) {
            ...
            const char *cc =
                scores[offs] > 99 || scores[offs + 1] > 99 ? "31" : "55";
            ...
            if (!offs) {
                ...
                int ft = 0;
                if ((flaginfos[0].state == CTFF_STOLEN &&
                     flaginfos[0].actor == p && flaginfos[0].ack) ||
                    (flaginfos[1].state == CTFF_STOLEN &&
                     flaginfos[1].actor == p && flaginfos[1].ack && ++ft)) {
    ...
    glLoadIdentity();
    glOrtho(0, VIRTW, VIRTH, 0, -1, 1);
    if (m_flags_ && player1->spectatemode == SM_OVERVIEW &&
        overviewflags == 2) {
        float iconsize = radarentsize / 0.2f;
        vec coordtrans =
            vec(VIRTW / (1.0f * clmapdims.xspan + 4),
                VIRTH / (1.0f * clmapdims.yspan + 4),
                1); // compare orthd for SM_OVERVIEW in setperspective()
        vec coordcenter = vec(clmapdims.x1 + clmapdims.xspan / 2,
                              clmapdims.y1 + clmapdims.yspan / 2, 0);
        bool axisxoff =
            clmapdims.xspan <
            clmapdims.yspan; // the axis of the not-gdim coordspan needs fixing
        float axisscale =
            (1000.0f / (axisxoff
                            ? (clmapdims.yspan / (1.0f * clmapdims.xspan))
                            : (clmapdims.xspan / (1.0f * clmapdims.yspan)))) /
            1000.0f; // not used if !axisdiff
        if (axisxoff) {
        ...
        loopi(2) // flag items
        {
            ...
            bool hasflagent =
                e && e->x != -1 && e->y != -1; // no base for flagentitydummies
                                               // (HTF on maps without bases)
            if (hasflagent) {
                vec pos = vec((e->x - coordcenter.x) * coordtrans.x,
                              (e->y - coordcenter.y) * coordtrans.y, 0);
                drawradarent(pos.x + VIRTW / 2, pos.y + VIRTH / 2, 0,
                             m_ktf ? 2 : f.team, 3, iconsize,
                             false); // draw bases
            }
            if ((f.state == CTFF_INBASE && hasflagent) ||
                f.state == CTFF_DROPPED) {
                vec cpos =
                    vec((f.pos.x - coordcenter.x) * coordtrans.x,
                        (f.pos.y - coordcenter.y) * coordtrans.y, f.pos.z);
                float flgoff = fabs(radarentsize * 2.1f);
                drawradarent(cpos.x + flgoff + VIRTW / 2,
                             cpos.y - flgoff + VIRTH / 2, 0, 3,
                             m_ktf ? 2 : f.team, iconsize,
                             false); // draw on entity pos or whereever dropped
            }
            if (f.state == CTFF_STOLEN) {
                ...
                if (f.actor) {
                    ...
                    if (tm) {
                        apos.x *= coordtrans.x;
                        apos.y *= coordtrans.y;
                        drawradarent(apos.x + VIRTW / 2, apos.y + VIRTH / 2, 0,
                                     3, m_ktf ? 2 : f.team, iconsize,
                                     true); // draw near flag thief
                    }
                }
            }
