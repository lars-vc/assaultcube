...
const char *mmshortname(const char *name) {
    return !strncmp(name, mmpath, strlen(mmpath)) ? name + strlen(mmpath)
                                                  : name;
}
...
void mapmodel(int *rad, int *h, int *zoff, char *scale, char *name,
        ...
        *name) // ignore "mapmodel" commands with insufficient parameters
    {
        ...
        formatstring(mmi.name)(
            "%s%s", mmpath, name[0] == '.' && name[1] == '/' ? name + 2 : name);
        mapmodelchanged = 1;
        flagmapconfigchange();
    } else {
        flagmapconfigerror(LWW_CONFIGERR * 2);
...
void sortmapmodelslots(char **args, int numargs) {
    ...
    loopv(ents) if (ents[i].type == MAPMODEL &&
                    tempslots.inrange(ents[i].attr2)) tempslots[ents[i].attr2]
        .used = true;
...
void setmodelattributes(const char *name, mapmodelattributes &ma) {
    ...
    mapmodelattributes **mrp = mdlregistry.access(name),
                       *mr = mrp ? *mrp : NULL, *r = mr;
    if (!r) {
...
void mapmodelregister_(
    char **args, int numargs) // read model attributes without loading the model
{
    ...
    if (mm)
        ...
        if (loadingmodel && m) {
            ...
            if (m->shadowdist && !m->cullface) {
                conoutf(
                    "\f3mapmodel config error: disabling face culling in "
                    "combination with shadows will cause visual errors (%s)",
                    m->name());
                flagmapconfigerror(LWW_MODELERR);
            }
        }
...
void updatemapmodeldependencies() {
    ...
    loopv(ents) if (ents[i].type == MAPMODEL &&
                    mapmodels.inrange(ents[i].attr2)) mapmodels[ents[i].attr2]
        .flags |= MMF_TEMP_USED;
    bool foundone = true;
    while (foundone) {
        foundone = false;
        loopvk(mapmodels) if (mapmodels[k].flags & MMF_TEMP_USED) {
            ...
            mapmodelattributes **ap = mdlregistry.access(name),
                               *a = ap ? *ap : NULL;
            const char *dep = a ? a->n[MMA_REQUIRES] : NULL;
            if (dep && *dep) {
                bool found = false;
                loopv(mapmodels) if (!strcmp(mmshortname(mapmodels[i].name),
                                             dep)) {
                ...
                if (!found) {
                    defformatstring(cmd)("mapmodel 0 0 0 0 \"%s\" %d", dep,
                                         MMF_REQUIRED);
                    execute(cmd);
                    foundone = true;
                    clientlogf(
                        " mapmodel %s added to config, because %s requires it",
                        dep, name);
                }
            }
        }
...
void writemapmodelattributes() {
    ...
    if (f) {
        ...
        mmas.sort(mmasort);
        f->printf("// automatically written on exit. this is cached "
                  "information extracted from model configs. no point in "
                  "editing it.\n// [path %s]\n",
        ...
        loopv(mmas) {
            ...
            loopj(MMA_NUM) f->printf(
                " %s",
                escapestring(mmas[i]->n[j])); // escapestring can handle NULL
        }
        f->printf("\n\n");
        delete f;
    }
...
void renderbatchedmodel(model *m, batchedmodel &b) {
    ...
    if (stenciling) {
        m->render(b.anim | ANIM_NOSKIN, b.varseed, b.speed, b.basetime, b.o,
                  b.roll, b.yaw, b.pitch, b.d, a, b.scale);
        return;
    }
    ...
    m->render(b.anim, b.varseed, b.speed, b.basetime, b.o, b.roll, b.yaw,
...
void rendermodel(const char *mdl, int anim, int tex, float rad, const vec &o,
    ...
    if (rad >= 0) {
        if (!rad) {
            ...
            if (roll != 0 || pitch != 0)
                rad = max(m->radius,
                          m->zradius); // FIXME: this assumes worst-case even
    ...
    if (stenciling) {
        m->startrender();
        m->render(anim | ANIM_NOSKIN, varseed, speed, basetime, o, roll, yaw,
                  pitch, d, a, scale);
        m->endrender();
        return;
    }
        ...
        if (!(anim & ANIM_TRANSLUCENT) && dynshadow && m->hasshadows() &&
            ...
            if (!dynshadowquad || center.z - 0.1f <= o.z) {
                ...
                if (dynshadowdecay)
                    ...
                    case ANIM_DECAY:
                    case ANIM_LYING_DEAD:
                        intensity *= max(1.0f - float(lastmillis - basetime) /
                                                    dynshadowdecay,
                                         0.0f);
                        break;
                    }
    ...
    if (anim & ANIM_TRANSLUCENT) {
        glColorMask(GL_FALSE, GL_FALSE, GL_FALSE, GL_FALSE);
        m->render(anim | ANIM_NOSKIN, varseed, speed, basetime, o, 0, yaw,
                  pitch, d, a, scale);
        glColorMask(GL_TRUE, GL_TRUE, GL_TRUE, GL_TRUE);
...
bool preload_mapmodels(bool trydl) {
    ...
    loopv(
        mapmodels) if ((mapmodels[i].flags & (MMF_REQUIRED | MMF_TEMP_USED)) &&
    ...
    if (d->state == CS_DEAD) {
        ...
        basetime = d->lastpain;
               ...
               (d->weaponsel->type != GUN_GRENADE ||
                (d == player1
                     ? ((grenades *)player1->weapons[GUN_GRENADE])->throwmillis
                     : ((grenades *)d->weapons[GUN_GRENADE])->cookingmillis >
                           0))) {
...
void renderclientp(playerent *d) {
    ...
    string vwep;
    if (d->weaponsel &&
        !(d->weaponsel->type == GUN_GRENADE && !d->mag[GUN_GRENADE].val()))
...
void renderclients() {
    ...
    if (player1->state == CS_DEAD ||
        ((reflecting && !refracting) &&
         (player1->state != CS_SPECTATE || player1->spectatemode == SM_FLY)))
