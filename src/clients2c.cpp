                   ...
                   int revision) // forced map change from the server
{
    ...
    if (disconnectonmaperror && incompatiblemap && multiplayer(NULL))
        trydisconnect();
    if (download > 0) {
        bool revmatch = hdr.maprevision == revision || revision == 0;
        if (watchingdemo) {
            if (loaded && !revmatch)
                conoutf("\f3demo was recorded on map revision %d, you have map "
                        "revision %d",
            ...
            if (loaded && sizematch && revmatch)
                return true;
            bool getnewrev =
                autogetnewmaprevisions && revision > hdr.maprevision;
            if (autogetmap || getnewrev) {
                ...
                if (!loaded || download < 10)
                    conoutf("\"getmap\" to download the current map from the "
                            "server");
                else
                    conoutf(
                        "\"getmap\" to download a %s version of the current "
                        "map from the server",
                        revision == 0
                            ? "different"
    ...
    while (p.remaining())
        ...
        case SV_POSC4: {
            ...
            if (type != SV_POS) {
                ...
                z = getuint(p);
                uint64_t ff =
                    getuintn(p, (YAWBITS + PITCHBITS + FLAGBITS + 7) / 8);
    ...
    while (p.remaining()) {
        ...
        case SV_SERVINFO: // welcome message from the server
        {
            ...
            if (prot != CUR_PROTOCOL_VERSION &&
                ...
                conoutf("\f3incompatible game protocol (local protocol: %d :: "
                        "server protocol: %d)",
            ...
            if (curpeer && iabs(clockoffset) > 60 * 24)
                conoutf("\f3warning: server and client clock are offset more "
                        "than one day (%d minutes)",
                        clockoffset);

            clientlogf("own IP: %s, censored own IP: %s, %s, clock offset %d "
                       "hours %d minutes",
                       iptoa(s->clientip, tmp1),
                       iptoa(s->clientipcensored, tmp2), cc, clockoffset / 60,
                       clockoffset % 60);
            if (curpeer && s->serverip) {
                s->curpeerip = ENET_NET_TO_HOST_32(curpeer->address.host);
                s->curpeerport = curpeer->address.port;
            }
            if (s->curpeerip && s->curpeerip != s->serverip)
                conoutf(
                    "\f3warning: server IP mismatch (true: %s, reported: %s)",
                    iptoa(s->curpeerip, tmp1), iptoa(s->serverip, tmp2));

            defformatstring(challenge)(
                "SERVINFOCHALLENGE<(%d) cn: %d c: %s (%s) s: %s:%d", sessionid,
                s->cn, iptoa(s->clientipcensored, tmp1), cc,
                iptoa(s->serverip, tmp2), s->curpeerport);
            concatformatstring(challenge, " %s st: %d ct: %d (%d)>",
                               bin2hex(tmp1, s->serverpubkey, 32),
                               s->serverclock, s->clientclock, s->clientsalt);
            clientlogf("auth challenge: %s", challenge);
            int chlen = (int)strlen(challenge);
            if (chlen >= MAXSTRLEN - 65) {
                conoutf(
                    "\f3ERROR: oversized auth challenge (%d), can't connect",
                    chlen);
                disconnect();
                return;
            }
            if (sk)
                ed25519_sign((uchar *)challenge, NULL, (uchar *)challenge,
                             chlen, sk);
        ...
        case SV_SERVINFO_CONTD: // 2nd welcome message from the server
        {
            ...
            int auth = getint(p);
            if (auth) {
                formatstring(servauth)(
                    "SERVINFOSIGNED<%s>",
                    bin2hex(tmp, (uchar *)s->clientsignature, 64));
                memmove(servauth + 64, servauth,
                        (auth = (int)strlen(servauth)) + 1);
...
#ifdef _DEBUG
                formatstring(text)("server-side%s",
                                   s->datecodes ? " timed flags:" : "");
                loopi(VS_NUMCOUNTERS) if (s->datecodes & (1 << i))
                    concatformatstring(text, " %s", vskeywords[i]);
                if (s->clan[0])
                    concatformatstring(text, " clan tag: \"%s\"", s->clan);
                if (s->publiccomment[0])
                    concatformatstring(text, " comment: \"%s\"",
                                       s->publiccomment);
                if (strlen(text) > 12)
                    conoutf("%s", text);
#endif
            ...
            if (curpeer && sk && !auth)
                conoutf("\f3server refuses to authenticate");
            if (sk && auth) {
                if (!ed25519_sign_check((uchar *)servauth, auth + 64,
                                        s->serverpubkey))
                    conoutf("\f3server authentication failed");
                else
                    clientlogf("server authenticated as %s",
                               bin2hex(tmp, s->serverpubkey, 32));
            }
        ...
        case SV_TEXTPRIVATE: {
            ...
            if (!s)
                break;
            if (s == player1 && d)
                conoutf("to %s (PM):\f9 %s", colorname(d), highlight(text));
            else if (s->ignored)
                clientlogf("ignored: pm %s %s", colorname(s), text);
            else {
        ...
        case SV_SWITCHNAME:
            ...
            if (d) {
                if (strcmp(d->name, text))
                    conoutf("%s is now known as %s", colorname(d),
                            colorname(d, text));
        ...
        case SV_SPAWN: {
            ...
            loopi(NUMGUNS) set_val_lars(s->mag[i], getint(p));
            s->state = CS_SPAWNING;
            arenaintermission = 0;
            if (s->lifesequence == 0)
                s->resetstats(); // NEW
        ...
        case SV_SPAWNSTATE: {
            ...
            loopi(NUMGUNS) set_val_lars(player1->mag[i], getint(p));
        ...
        case SV_DAMAGE: {
            ...
            if (!target || !actor || !valid_weapon(gun))
                break;
        ...
        case SV_RESUME: {
            ...
            loopi(MAXCLIENTS) {
                ...
                if (d != player1) {
                    ...
                    printf("START\n");
                    loopi(NUMGUNS) printf("%d\n", d->mag[i].val());
                    loopi(NUMGUNS) printf("%d\n", mag[i].val());
                    printf("END\n");
                    loopi(NUMGUNS) set_val_lars(d->mag[i], mag[i].val());
        ...
        case SV_SETTEAM: {
            ...
            if (d) {
                ...
                if (m_teammode || team_isspect(fnt)) {
                    if (d->team == fnt) {
                        ...
                        if (you && !watchingdemo) {
                            ...
                            case FTR_PLAYERWISH:
                                ...
                                break;
                            case FTR_AUTOTEAM:
                                hudoutf("the server forced you to team %s",
                                        nts);
                                break;
                            }
                        } else {
                            ...
                            bool et =
                                team_base(player1->team) != team_base(fnt);
                            switch (ftr) {
                            case FTR_PLAYERWISH:
                            ...
                            case FTR_AUTOTEAM:
                                if (watchingdemo || team_isspect(player1->team))
                                    conoutf("the server forced %s to team %s",
                                            colorname(d), nts);
                                else
                                    hudoutf("the server forced %s to %s team",
                                            colorname(d),
                                            et ? "the enemy" : "your");
                                break;
                            }
                    ...
                    conoutf("%s changed to active play",
                            you ? "you" : colorname(d));
                d->team = fnt;
        ...
        case SV_SENDDEMOLIST: {
            ...
            if (!demos) {
                ...
                loopi(demos) {
                    ...
                    concatformatstring(text, ", %.2f%s",
                                       len > 1024 * 1024 ? len / (1024 * 1024.f)
                                                         : len / 1024.0f,
                                       len > 1024 * 1024 ? "MB" : "kB");
        ...
        case SV_DEMOCHECKSUM: // raw data checksum of demo file
        {
            ...
            if (p.get(dtiger, TIGERHASHSIZE) == TIGERHASHSIZE) {
                ...
                defformatstring(msg)("CLIENTDEMOEND<%s %d>",
                                     iptoa(session.curpeerip, text + 100),
                                     session.curpeerport);
                int msglen = (int)strlen(msg);
                ASSERT(msglen + 64 < MAXSTRLEN);
                if (sk)
                    ed25519_sign((uchar *)msg, NULL, (uchar *)msg, msglen, sk);
                else
                    memset(msg, 0, 64);
        ...
        case SV_VITADATA: { // Server sending vita data of a client. Possibly
                            // expose this to cubescript in the future
            uchar pubkey[32];
            char pubkeyhex[68];
            ...
            if (target && p.get(pubkey, 32) == 32) {
                ...
                conoutf("\f0Vita Info for \f5%s \f0(\f5%d\f0) \f2pubkey: \f1%s",
                        target->name, targetcn, pubkeyhex);
                if (privatecomment[0])
                    conoutf("\f0Private Comment: \f1%s", privatecomment);
                if (publiccomment[0])
                    conoutf("\f0Public Comment: \f1%s", publiccomment);

                string msg = "\f0Flags:";
                for (int i = 0; i < VS_NUM; i++) {
                    concatformatstring(msg, " \f2%s=\f1%d", vskeywords[i],
                                       vs[i]);
                }
                conoutf("%s", msg);
            }
...
void receivefile(uchar *data, int len) {
    ...
    case SV_SENDDEMO: {
        ...
        if (demosize <= MAXDEMOSENDSIZE && demosize <= p.remaining()) {
            ...
            defformatstring(fname)(
                "demos/%s%s.dmo", demosubpath,
                formatdemofilename(demonameformat, demotimestampformat, text,
                                   gmode, timestamp, timeplayed, timeremain, ip,
                                   buf));
    ...
    case SV_RECVMAP: {
        ...
        if (mapsize <= MAXMAPSENDSIZE && cfgsizegz <= MAXMAPSENDSIZE &&
            size <= MAXMAPSENDSIZE && cfgsize <= MAXCFGFILESIZE &&
            size <= p.remaining() && !securemapcheck(behindpath(text))) {
