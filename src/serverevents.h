...
void processevent(client *c, shotevent &e) {
    ...
    if (!gs.isalive(sg->gamemillis) || !valid_weapon(e.gun) ||
        wait < gs.gunwait[e.gun] || gs.mag[e.gun].val() <= 0)
        return;

    if (e.gun == GUN_AKIMBO && gs.akimbomillis < sg->gamemillis)
        return;
    if (e.gun != GUN_KNIFE)
        gs.mag[e.gun]--;
    ...
    if (e.gun == GUN_PISTOL && gs.akimbomillis > sg->gamemillis)
        gs.gunwait[e.gun] /= 2;
    ...
    gs.shotdamage +=
        ...
        (e.gun == GUN_SHOTGUN
             ...
             : 1); // 2011jan17:ft: so accuracy stays correct, since
                   // SNIPER:headshot also "exceeds expectations" we use
...
void processevent(client *c, pickupevent &e) {
    clientstate &gs = c->state;
    if (m_mp(gamemode) && !gs.isalive(sg->gamemillis))
        return;
...
void processevent(client *c, reloadevent &e) {
    ...
    int mag = (akimbo ? 2 : 1) * magsize(e.gun),
        numbullets = min(gs.ammo[e.gun], mag - gs.mag[e.gun].val());
    if (numbullets <= 0)
        return;
...
void processevent(client *c, akimboevent &e) {
    clientstate &gs = c->state;
    if (!gs.isalive(sg->gamemillis) || gs.akimbomillis)
        return;
...
void processevents() {
    loopv(clients) {
        ...
        if (c->type == ST_EMPTY)
            continue;
        if (c->state.akimbomillis && c->state.akimbomillis < sg->gamemillis) {
        ...
        while (c->events.length()) {
            ...
            if (e.type < GE_SUICIDE) {
                if (e.shot.millis > sg->gamemillis)
                    break;
