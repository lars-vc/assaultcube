...
char *colorname(playerent *d, char *name, const char *prefix) {
    ...
    if (name[0] && !isdname && !getigraph(name + 1))
        return name;
    ...
    num = (num + 1) % 4;
    if (!name[0] || isdname)
        formatstring(cnum)(" \f6(%d)", d->clientnum);
...
char *colorping(int ping) {
    static string cping;
    formatstring(cping)("\fs\f%d%d\fr",
                        ping <= 500    ? 0
                        : ping <= 1000 ? 2
                                       : 3,
                        ping);
...
void clearignored(int *cn) {
    loopv(players) if (players[i] && (*cn < 0 || *cn == i)) players[i]
        ->ignored = false;
}
...
COMMANDF(
    curpausemode, "", (void) { intret(ispaused); });
COMMANDF(
...
void playerinfo(int *cn, const char *attr) {
    ...
    if (!p) {
        if (!m_botmode &&
            multiplayer(
                NULL)) // bot clientnums are still glitchy, causing this message
                       // to sometimes appear in offline/singleplayer when it
    ...
    if (p == player1) {
        ATTR_INT(magcontent, p->weaponsel->mag.val());
...
const char *currentserver(int i) // [client version]
{
    ...
    if (curpeer) {
        ...
        // or the scoreboard shown SERVERNAME
        case 5: {
...
void deathstate(playerent *pl) {
    ...
    if (pl == player1) {
        ...
        if (pl->spectatemode == SM_DEATHCAM)
            addmsg(SV_SPECTCN, "ri", (player1->followplayercn = FPCN_DEATHCAM));
    } else
        pl->resetinterp();
}
...
void movelocalplayer() {
    if (ispaused && player1->state == CS_ALIVE)
        return;
...
void moveotherplayers() {
    loopv(players) if (players[i] && players[i]->type == ENT_PLAYER) {
        ...
        if (!lagtime || intermission || ispaused)
            continue;
...
void showrespawntimer() {
    if (intermission || ispaused || spawnpermission > SP_OK_NUM)
        return;
    if (m_arena) {
    ...
    } else if (player1->state == CS_DEAD && m_flags_ &&
                ...
                spawncycle =
                    clentstats.hasffaspawns
                        ...
                        : findentity(PLAYERSTART,
                                     spawncycle +
                                         1); // if not enough ffa-spawns are
                                             // available, use all
                if (spawncycle < 0)
                    continue;
                float dist =
...
inline const char *spawn_message() {
    ...
    // AssaultCube itself has outsold the Encyclopedia Galactica because it is
    // slightly cheaper, and because it has the words "Don't Panic" in large,
    // friendly letters.
}
...
bool tryrespawn() {
    if (spawnpermission > SP_OK_NUM) {
        ...
        if (team_isspect(player1->team)) {
            ...
            int respawnmillis =
                player1->lastdeath + (m_arena ? 0 : (m_flags_ ? 5000 : 2000));
...
COMMANDF(
    fragmessage, "ss", (const char *gun, const char *message) {
        setkillmessage(gun, false, message);
    });
COMMANDF(
    gibmessage, "ss", (const char *gun, const char *message) {
...
COMMANDF(
    burstshots, "si", (char *gun, int *shots) {
        ...
        if (guni >= 0 && guns[guni].isauto) {
            if (*shots >= 0)
                burstshotssettings[guni] =
                    min(*shots, (guns[guni].magsize - 1));
            else
...
void dodamage(int damage, playerent *pl, playerent *actor, int gun, bool gib,
              bool local) {
    if (pl->state != CS_ALIVE || intermission || ispaused)
        return;
...
void timeupdate(int milliscur, int millismax) {
    ...
    if (!minutesremaining) {
        ...
        extern int gametimedisplay; // only output to console if no hud-clock
                                    // with game time is being shown
        int sec =
            60 -
              ...
              bool norespawn) // called just after a map load
{
    ...
    editingsettingsshowminimal =
        true; // 1st edittoggle shows edithideentmask(!=0); turned off health
              // pickups last session 2 months ago? now you remember!
    ...
    if (mapstartonce && mapstartonce[0]) {
        addsleep(0,
                 mapstartonce); // do this as a sleep to make sure map changes
...
void flagmsg(int flag, int message, int actor, int flagtime) {
    ...
    case FM_KTFSCORE: {
        ...
        int m = flagtime / 60;
        if (m)
            hudoutf("\f2%s%s%s kept the flag for %d min %d s now", tc, ta, tb,
                    m, flagtime % 60);
        else
            hudoutf("\f2%s%s%s kept the flag for %d s now", tc, ta, tb,
                    flagtime);
        break;
    }
...
void cmd_pause(int *arg1) {
    ...
    if (servstate.mastermode != MM_MATCH &&
        servstate.mastermode != MM_PRIVATE) {
...
void whois(int *cn) {
    ...
    loopv(players) if (players[i] && players[i]->type == ENT_PLAYER &&
        ...
        conoutf("WHOIS client %d:\tname %s , IP %s , %d teamkill%s",
                p->clientnum, colorname(p), iptoa(p->address, tmp), p->tks,
                p->tks == 1 ? "" : "s"); // full IP
    }
}
COMMAND(whois, "i");

void getvita(int *cn) { addmsg(SV_GETVITA, "ri", *cn); }
COMMAND(getvita, "i");
...
playerent *updatefollowplayer(int shiftdirection) {
    ...
    loopv(players) if (players[i]) {
        if (team_group(players[i]->team) == omit_team)
            continue; // if not team SPECT and not watchingdemo and teammode:
                      // don't spectate enemy team
        if ((players[i]->state == CS_DEAD || players[i]->isspectating()) &&
            (!stayondeadplayers || players[i] != f))
            continue; // don't spect dead players, but in some cases stay on
                      // them
    ...
    if (player1->followplayercn != available[idx]->clientnum)
        addmsg(SV_SPECTCN, "ri", available[idx]->clientnum);
...
void setfollowplayer(int cn) {
    ...
    if (players.inrange(cn) && players[cn] && !m_botmode) {
        if (!(m_teammode && player1->team != TEAM_SPECT && !watchingdemo &&
            ...
            if (player1->followplayercn != cn)
                addmsg(SV_SPECTCN, "ri", (player1->followplayercn = cn));
...
// SM_OVERVIEW shows all players and their activity - could be used unfairly,
// possible TODO: allow restricting access by server config and votes
...
void spectatemode(int mode) {
    ...
    if (mode == player1->spectatemode)
        return;
    if ((mode == SM_OVERVIEW || mode == SM_FLY) && smoverviewflyforbidden())
        return;
    showscores(false);
    switch (mode) {
    case SM_DEATHCAM:
    ...
    case SM_FOLLOW3RD_TRANSPARENT: {
        if (players.length() && updatefollowplayer())
            break;
        else if (smoverviewflyforbidden())
            mode = SM_DEATHCAM;
        else
            mode = SM_FLY;
    }
            ...
            if (f) {
                ...
                if (collide(&d, false)) // need air around
                {
                    ...
                    if (x >= 0) {
                        ...
                        player1->o.z =
                            S((int)player1->o.x, (int)player1->o.y)->floor +
                            player1->maxeyeheight;
                        entinmap(player1);
                    } else {
    ...
    if (player1->spectatemode == SM_NONE)
        ...
        mode = SM_FOLLOW1ST +
               ((player1->spectatemode - SM_FOLLOW1ST + 1) %
                ((smoverviewflyforbidden() ? SM_FLY : SM_NUM /*SM_OVERVIEW*/) -
                 SM_FOLLOW1ST)); // replace SM_OVERVIEW by SM_NUM to enable
                                 // overview mode
    spectatemode(mode);
}
COMMAND(togglespect, "");

void changefollowplayer(int *shift) { updatefollowplayer(*shift); }
